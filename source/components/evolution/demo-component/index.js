...
export default function(){
    ...
    let demoItems = document.querySelectorAll('.evo_c-demo-help');
    let i, j, k, l;
    let demoItemsArray = [];
    ...
    if(demoItems.length > 0){ //ensure that there are demo items on the page
        ...
        for(j = 0; j < demoItemsArray.length; j++){
            demoItemsArray[j].innerHTML = '<span class="evo_c-demo-help__message-icon"><i class="material-icons">help_outline</i></span>' + demoItemsArray[j].innerHTML;

            /* Ensure that help message shows within the bounds of the screen */

            let helpMessage = demoItemsArray[j].querySelector('.evo_c-demo-help__content');
        ...
        let prevButtonList = document.querySelectorAll('.evo_c-demo-help__controls-prev');
        let nextButtonList = document.querySelectorAll('.evo_c-demo-help__controls-next');

        for (k = 0; k < prevButtonList.length; k++){

            (function(){
                prevButtonList[k].addEventListener('click', function () {
                    let parent = findAncestor(this, 'evo_c-demo-help');
                    showPrevious(parent);
                });
            })();
        }
    ...
    function showHelp(){
        let block = this;
        let message = block.querySelector('.evo_c-demo-help__content');
    ...
    function showPrevious(node){
        ...
        if(prevIndex > 0){
            let message = node.querySelector('.evo_c-demo-help__content');
            message.classList.remove('is-hidden');
            let previousMessage = demoItemsArray[prevIndex-1].querySelector('.evo_c-demo-help__content');
    ...
    function showNext(node){
        ...
        if(nextIndex > -1 && nextIndex < demoItemsArray.length - 1){
            let message = node.querySelector('.evo_c-demo-help__content');
            message.classList.remove('is-hidden');
            let nextMessage = demoItemsArray[nextIndex+1].querySelector('.evo_c-demo-help__content');
